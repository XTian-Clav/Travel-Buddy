...
function create_predefined_trip(PDO $pdo, int $user_id, array $packageData, string $destination, string $start_date, string $end_date, int $travelers, int $total_days): void {
    $activities = $packageData["activities"] ?? [];
    $total_activities = count($activities);
    $estimated_budget = (float)($packageData["budget"] ?? 0);
    $itinerary_name = $packageData["name"] ?? "Palawan Trip";
...
function get_predefined_package_data(string $package_key): array {
    switch ($package_key) {
